...
const getCorrectWordsHighlighted = (
  wordsCorrectlyTyped,
  expectedWords,
  typedWordsArray
) => {
  ...
  // index from question passage
  let lastMatchedCorrectWordIndexFromQuestionPassage = 0;

  // index from typed words passage
  let lastMatchedCorrectWordIndexFromTypedWords = 0;

  const lastCorrectWord = wordsCorrectlyTyped.slice(-1)[0];
  ...
  while (
    ...
    if (currentExpectedWord === currentCorrectWord) {
      ...
      lastMatchedCorrectWordIndexFromQuestionPassage = expectedWordsIndex;
      typedWordIndex += 1;
      correctWords += 1;
  ...
  if (lastCorrectWord) {
    for (
      let index = 0;
      index < typedWordsArray.length &&
      index <= lastMatchedCorrectWordIndexFromQuestionPassage;
      index += 1
    ) {
      if (typedWordsArray[index] === lastCorrectWord) {
        lastMatchedCorrectWordIndexFromTypedWords = index;
        console.log({
